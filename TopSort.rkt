...
(define (indegree Es v)
  (cond [(null? Es) 0]
        [(equal? (car(cdr(car Es))) v) (+ 1 (indegree (cdr Es) v))]
...
(define (find-top G)
  (cond [(null? (get-vertices G)) (tsort-error)]
        [(zero? (indegree (get-edges G) (car(get-vertices G)))) (car(get-vertices G))]
...
(define (tsort G)
  (cond [(null? (get-vertices G)) '()]
        [else (cons (find-top G) (tsort (pop-vertex G (find-top G))))]))
