...
(define (union LA LB)
  (make-set(cond ((null? LA) LB)
                 ((null? LB) LA)
                 ((element? (car LA) LB) (union (cdr LA) LB))
                 (else (cons (car LA) (union (cdr LA) LB)))
  )))
...
(define (intersection LA LB)
  (make-set (cond ((null? LA) '())
                  ((null? LB) '())
                  ((element? (car LA) LB) (cons (car LA) (intersection (cdr LA) LB)))
...
(define (subset? SA SB)
  (cond ((null? SA) #t)
        ((> (length SA) (length SB)) #f)
        ((element? (car (make-set SA)) (make-set SB)) (subset? (cdr (make-set SA)) (make-set SB)))
        (else #f)))
...
(define (set-difference LA LB)
  (make-set(cond((null? LA) '())
       ((null? LB) LA)
       ((element? (car LA) LB) (set-difference (cdr LA) LB) (set-difference (cdr LA) LB))
       (else (cons (car LA) (set-difference (cdr LA) LB)))
      ))
)
