...
(define (get-vertices G)
  (usort (makeVertList G)))

(define (makeVertList G)
  (cond [(null? G) '()]
        [else (append (car G) (makeVertList (cdr G)))])
  )
...
(define (degree G v)
  (define sum 0)
  (cond [(null? G) sum]
        [(and (equal? (car(car G)) v) (equal? (car(cdr(car G))) v)) (+ sum 2 (degree (cdr G) v))]
        [(equal? (car(car G)) v) (+ sum 1 (degree (cdr G) v))]
        [(equal? (car(cdr(car G))) v) (+ sum 1 (degree (cdr G) v))]
        [else (degree (cdr G) v)])
  )
...
(define (all-even? G)
  (define (evenList G)
  (map (lambda (b) (even? (degree G b)))
       (get-vertices G)))
  (define (reduceList G)
    (cond [(null? G) #t]
          [else (and (car G) (reduceList (cdr G)))]))
  (reduceList (evenList G))
  )
...
(define (adjacent G v)
  (define (adjList G v L)
  (cond [(null? G) L]
        [(equal? (car(car G)) v) (append L (list(car(cdr(car G)))) (adjList (cdr G) v L))]
        [(equal? (car(cdr(car G))) v) (append L (list(car(car G))) (adjList (cdr G) v L))]
...
(define (connected? G)
  (cond [(null? G) #t]
        [(equal? (usort(dfs G '() (car (get-vertices G)))) (get-vertices G)) #t]
        [else #f]))
