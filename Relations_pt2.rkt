...
(define (compose RO RI)
  (if (null? RO) '()
      (append (CH (car RO) RI) (compose (cdr RO) RI))))

(define (CH q RI)
  (if (null? RI) '()
      (if (eq? (car(cdr(car RI))) (car q))
          (cons (list (car(car RI)) (car(cdr q))) (CH q (cdr RI)))
          (CH q (cdr RI)))))
...
(define (PH R P k)
  (if (eq? k 1) P (PH R (compose R P) (- k 1))))
...
(define (TCH R P L)
  (if (eq? L 0) '() (if (eq? L 1) P (union P (TCH R (power R L) (- L 1))))))
...
(define (EQ-relation? R n)
  (if (reflexive? R n) (if (symmetric? R) (if (transitive? R) #t #f) #f) #f))
